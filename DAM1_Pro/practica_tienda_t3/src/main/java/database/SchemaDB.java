package database;

public class SchemaDB {
    public static final String TAB_TRABAJADORES = "trabajadores";
    public static final String COL_ID_TRABAJADOR = "id";
    public static final String COL_NOMBRE = "nombre";
    public static final String COL_CARGO = "cargo";
    public static final String COL_SALARIO = "salario";
    public static final String COL_CORREO = "correo";
    public static final String COL_PASSWORD = "password";

    public static final String TAB_PRODUCTOS = "productos";
    public static final String COL_ID_PRODUCTO = "id";
    public static final String COL_PRECIO = "precio";
    public static final String COL_CANTIDAD = "cantidad_stock";
}
